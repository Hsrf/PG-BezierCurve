...
*/
    ...
    var canvas, context, controlPointXArray, controlPointYArray, currentCurve, amountCurves, curveArrayX, curveArrayY, amountEvaluations, showControlPoints, showCurves, showLines, selectionBool, selectedPointIndex, userX, userY, deletionBool;
    ...
    function drawPoint(){
        ...
        if(showControlPoints){
            context.beginPath();
            context.arc(userX, userY, pointRadius, 0, Math.PI * 2);
            context.fill();
        }
        ...
        if(arraySize >= 1){ 
            drawLine(userX, userY);
        }

        //Adiciona o novo ponto ao array de pontos
        controlPointXArray[currentCurve].push(userX);
        controlPointYArray[currentCurve].push(userY);
    ...
    function newCurve(){
        ...
        canvas.addEventListener("mousedown", checkToSelectOrDraw);
        updateCountersOnDisplay()
    }
    ...
    function reDrawExistingPoints(){
        ...
        for(var i = 1; i < controlPointXArray.length; i++){
            for(var j = 0; j < controlPointXArray[i].length; j++){
                context.beginPath();
                context.arc(controlPointXArray[i][j], controlPointYArray[i][j], 5, 0, Math.PI * 2);
                context.fill();
                context.stroke();
            }
        }
   ...
   function hideUnChecked(){
    ...
    function checkToSelectOrDraw(event){
        //Tres possibilidades a partir daqui: desenhar ponto, mover ponto ou deletar ponto
        ...
        if(!selectionBool){
            ...
            if(deletionBool){
                //Deleta o ponto
                deletePointNEarby(userX, userY);
            }else{
                //Move o ponto
                var index = getIndexNearbyPoint(userX, userY);
                if(index != -1){
                    //Isso significa que  existe um ponto no nearby na posicao 'index' do array de curva
                    //THIS IS WHERE I WOULD MOVE POINTS, IF I COULD DO THAT  *PAI DO TIMMY TURNER INTENSIFIES*
                    //waitForNextClick();
                    //curveArrayX[currentCurve][index] = userX;
                    //curveArrayY[currentCurve][index] = userY;
                }
            }
        }
    }
    ...
    function waitForNextClick(){
        var oldX = userX;
        var oldY = userY;
        console.log("Valores antigos: X: "+oldX + " Y: "+ oldY);

        while(userX == oldX){
            setTimeout(reDrawExistingObjects, 1000);
        }
        console.log("Valores novos: X: "+userX + " Y: "+ userY);

    }

    function togglePointSelection(event){
        selectionBool = !selectionBool;
    }

    function toggleDeletionBool(){
        deletionBool = !deletionBool;
    }

    function getIndexNearbyPoint(x, y){
        for(var i = 0; i < controlPointXArray[currentCurve].length; i++){
            //Checa se o ponto onde o usuario clickou possui algum ponto ja desenhado a '5' unidades de distancia
            if(controlPointXArray[currentCurve][i] >= x - 5 && controlPointXArray[currentCurve][i] <= x + 5 && controlPointYArray[currentCurve][i] >= y - 5 && controlPointYArray[currentCurve][i] <= y + 5){
    ...
    function deletePointNEarby(x, y){
        for(var i = 0; i < controlPointXArray[currentCurve].length; i++){
            //Checa se o ponto onde o usuario clickou possui algum ponto ja desenhado a '5' unidades de distancia
            if(controlPointXArray[currentCurve][i] >= x - 5 && controlPointXArray[currentCurve][i] <= x + 5 && controlPointYArray[currentCurve][i] >= y - 5 && controlPointYArray[currentCurve][i] <= y + 5){
                //console.log("Esta perto do ponto: " + i);
                controlPointXArray[currentCurve].splice(i, 1);
                controlPointYArray[currentCurve].splice(i, 1);
                clearCanvas();
                drawBezier();
                reDrawExistingObjects();
                return i;
